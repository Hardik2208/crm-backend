...
router.put("/Customer/Reassign/:id", async (req, res) => {
  try {
    const assignedCustomer = await Customer.findOne({
      phoneNumber: req.body.assignedNumber,
    });
    if (!assignedCustomer) {
    ...
    const orderToMove = customer.orderList.filter(
      (order) => order.orderNumber === req.body.orderNumber
    );
    if (orderToMove.length === 0) {
      return res.status(400).send("Order not found in current customer");
    }

    assignedCustomer.orderList = [
      ...assignedCustomer.orderList,
      ...orderToMove,
    ];
    customer.orderList = customer.orderList.filter(
      (order) => order.orderNumber !== req.body.orderNumber
    );
